...
class CSIRealtimeMonitorV5:
    def __init__(
        ...
        self.current_smoothed_state = "warming_up"
        self.current_smoothed_state_since = time.time()
        self.raw_decision_history = deque(maxlen=self.decision_history_size)
    ...
    def infer_raw_state(self, features):
        now_ts = time.time()
        ...
        self.last_valid_state = raw_state
        self.last_valid_state_time = now_ts
        return raw_state

    def smooth_state(self, raw_state):
        now_ts = time.time()

        if raw_state in ("calibrating", "no_signal", "warming_up"):
            return raw_state

        self.raw_decision_history.append(raw_state)

        if len(self.raw_decision_history) == 0:
            return self.current_smoothed_state

        counts = Counter(self.raw_decision_history)
        majority_state = counts.most_common(1)[0][0]

        if self.current_smoothed_state in ("warming_up", "calibrating", "no_signal"):
            self.current_smoothed_state = majority_state
            self.current_smoothed_state_since = now_ts
            return self.current_smoothed_state

        if majority_state != self.current_smoothed_state:
            if now_ts - self.current_smoothed_state_since >= self.min_state_duration:
                self.current_smoothed_state = majority_state
                self.current_smoothed_state_since = now_ts
    ...
    def run(self):
        ...
        print(f"Decision history={self.decision_history_size} Min state duration={self.min_state_duration}s")
        if self.target_mac:
            ...
            while True:
                ...
                if now_ts - self.last_inference_time >= self.step_seconds:
                    ...
                    if not self.calibrated:
                        ...
                        raw_state = self.infer_raw_state(features)
                        state = self.smooth_state(raw_state)

                        if features is None:
                            print(
                                f"state={state} raw_state={raw_state} "
                                f"kept={self.kept_lines} ignored={self.ignored_lines}"
                            )
                        else:
                            print(
                                f"state={state} raw_state={raw_state} "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min-state-duration", type=float, default=2.0)
    args = parser.parse_args()

    monitor = CSIRealtimeMonitorV5(
        port=args.port,
        ...
        decision_history_size=args.decision_history_size,
